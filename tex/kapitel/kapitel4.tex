...
\subsubsection{File System API und IndexedDB}
\bild{architekturen/architektur-file-system-api-indexed-db}{14cm}{Architektur mit File System API und IndexedDB}
...
Außerdem unterstützt jeder relevante Browser diese Funktion \autocite{mdn-fetch}. Die Fetch-API kann sowohl in Service Workern, als auch im \ac{DOM} verwendet werden \autocite{mdn-fetch}. Damit eine Netzwerkanfrage abgeschickt und empfangen werden kann muss der Browser aktiv sein. Sobald der Browser geschlossen wird kann keine Netzwerkanfrage mehr versendet werden, weil dann auch keine Service Worker mehr aktiv sind.

Ein Service Worker kann vom Browser zu jeder Zeit beendet werden, selbst wenn ein Event noch nicht beendet wurde \autocite{service-worker-spec}. Die Ausführungsdauer für ein Event sollte sehr kurz sein und ist nicht für langanhaltende Downloads gedacht \autocite{service-worker-spec}.

\subsection{HttpClient}
In Angular können Netzwerkanfragen mit dem HttpClient verschickt werden \autocite{httpClient}. Bei einem Request kann die URL, Header, Parameter und vieles mehr angepasst werden \autocite{httpClient}. Die Antwort eines Request wird als Observable zurück gegeben, auch mit der Option zwischendurch einen Fortschritt des Requests abzufragen \autocite{httpClient}. Dadurch, dass der HttpClient teil von Angular ist funktioniert er in allen relevanten Browsern. 

Der HttpClient kann in allen Angular Komponenten und Services per Dependecie Injection genutzt werden \autocite{httpClient}. Service Worker können beliebig zur Anwendung hinzugefügt werden aber angularspezifische Teile können nicht verwendet werden, somit ist auch der HttpClient in service workern nicht nutzbar \autocite{service-worker-angular}.
...
\autoref{diagramme/background-sync} zeigt den Ablauf wenn ein Sync-Event gesendet wird. Wenn bereits eine Netzwerkverbindung besteht wird das Event sofort ausgeführt. Ansonsten wird auf eine Netzwerkverbindung gewartet. Das Ausführen des Event kann zum Beispiel durch einen Netzwerkfehler scheitern. Wenn nach mehrmaligen Versuchen das Event nicht geklappt hat wird ein \textit{lastChance} Event gefeuert \autocite{Rojas2020}.

Diese Funktionalität ist Teil von Service Workern und funktioniert solange ein Service Worker läuft \autocite{wicg-background-sync}. Auch das Event für Background Sync hat nur eine begrenzte Lebensdauer und kann jederzeit vom Browser beendet werden \autocite{service-worker-spec}.
...
Um alle relevanten Browser abzudecken muss die Fetch \ac{API} oder der HttpClient verwendet werden. Da die Fetch \ac{API} auch in einem Service Worker verwendet werden kann, wird diese ausgewählt. 

Als Ergänzung kann später Background Fetch implementiert werden, weil damit auch Inhalte heruntergeladen werden können wenn die Website geschlossen ist. Background Sync bietet in diesem Fall keine großen Vorteile gegenüber der Fetch \ac{API}, weil die Laufzeit beider \acp{API} gleich ist. Der Vorteil bei Background Sync wäre das automatische Starten des Downloads sobald nach einem Verbindungsabbruch die Verbindung wiederhergestellt ist. Diese Funktionalität kann aber auch in Angular implementiert werden.
